document.querySelector('.grid').addEventListener('contextmenu', function(e) {
  e.preventDefault();//no menu occurs on right click
  if (e.target.tagName === 'IMG') {
    ...
    e.target.addEventListener('mouseout', function handler(d) {
     ...
     e.target.addEventListener('mousemove', function(f) {
      ...
      myElement.style.top = f.offsetY + 15 + "px";//so that the preview appears slightly below the cursor
    }, false);
...
//added a mousemove event so that the preview moves with the mouse.